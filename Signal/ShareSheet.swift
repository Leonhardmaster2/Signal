...
final class TracePackageExporter {
    ...
    /// Create a Trace package (.traceapp) that includes audio, transcript, and metadata
    /// Returns a zipped file URL for sharing
    func createTracePackage(recording: Recording) -> URL? {
        ...
        let packageName = "\(recording.title).traceapp"
        ...
        This .traceapp package contains:
        ...
        Import this .traceapp package into Trace to restore the complete recording with all its data.
        ...
        let zipFileName = "\(recording.title).traceapp"
        let zipURL = tempDir.appendingPathComponent(zipFileName)
        
        if let zippedURL = createZipArchive(from: packageURL, to: zipURL) {
            print("✅ Created .traceapp package: \(zippedURL.path)")
            return zippedURL
        } else {
            print("❌ Failed to create ZIP, returning directory")
            return packageURL
        }
    }
    ...
    /// Import a Trace package (.traceapp, .traceapp.zip, or legacy .traceaudio/.trace) and restore the recording
    @MainActor
    func importTracePackage(from url: URL, modelContext: ModelContext) async -> Bool {
        ...
        print("📦 Full filename: \(url.lastPathComponent)")
        ...
        if exists && isDirectory.boolValue {
            ...
            do {
                ...
                print("📦 Copied .traceapp directory to: \(tempTrace.path)")
            } catch {
                print("❌ Failed to copy .traceapp directory: \(error)")
            }
        } else if exists && !isDirectory.boolValue {
            // It's a flat file — could be a zip or a file-based .traceapp/.traceaudio/.trace package
            // Try to unzip it first
            let ext = url.pathExtension.lowercased()
            let fileName = url.lastPathComponent.lowercased()
            let isTraceFile = ext == "zip" || ext == "traceapp" || ext == "traceaudio" || ext == "trace" || 
